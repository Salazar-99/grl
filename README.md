...
- **GPU groups** (`rollouts`, `training`) carry the `nvidia.com/gpu` taint, so only GPU pods land on them. `N` is the GPUs advertised per node — resolved by the launcher from `compute.<role>.instance_type` via the AWS provider in [`config/src/grl_config/providers/aws.py`](config/src/grl_config/providers/aws.py). Multi-GPU training is not yet implemented (see ARCHITECTURE.md).
...
- **Sizing** (instance type, disk, node count) for each role is set in the top-level `compute` section of the launcher config; the launcher derives EKS `node_groups`, Ray worker replicas, and actor counts from it.
