...
#[cfg(target_os = "linux")]
pub async fn connect_executor(guest_cid: u32, timeout: Duration) -> Result<ExecutorConn, String> {
    ...
    loop {
        ...
        if remaining.is_zero() {
            return Err(format!("vsock connect cid={guest_cid} timed out"));
        }
        // Keep each blocking kernel operation short. If the async boot task is
        ...
        let result = tokio::task::spawn_blocking(move || {
            ExecutorConn::connect_vsock_timeout(guest_cid, attempt_timeout)
        })
        .await
