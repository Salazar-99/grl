...
**Task catalog.** At startup the manager loads `tasks.jsonl` from `GRL_TASKS_FILE` (synced by the DaemonSet initContainer from `GRL_BUNDLE_URI` into `{GRL_VM_CACHE_DIR}/active/`). Each line includes `task_id`, `split`, opaque `task_payload_json`, and node-relative VM image paths (`base_image`, `task_image`). Trainers call `ListTasks` for the task index (optionally filtered by split). On `CreateEnvironment` the manager boots a Firecracker microVM, connects to the guest, sends `InitializeRequest`, and returns the guest's initial messages/tools. Legacy `messages` and `tools` fields are accepted for migration and boot-disabled tests, but the manager does not interpret task-specific payload fields.

**VM boot and initialization.** On Linux, `CreateEnvironment` resolves `base_image` and `task_image` under `GRL_VM_CACHE_DIR`, attaches them as root and secondary drives, waits for the in-VM executor on vsock port 5005, sends exactly one `InitializeRequest`, and marks the session ready only after a valid response. Boot or initialization failure tears down the VM and returns an infrastructure error. Set `GRL_VM_BOOT=0` to use the legacy catalog response in local dev/tests. Optional `GRL_USE_JAILER=1` runs Firecracker via the jailer wrapper.
...
**Evaluate.** `Evaluate(env_id, final_message_json, termination_reason)` returns the task reward. The manager relays the request and response; the reward is computed inside the env executor. A valid model answer with reward zero is not an infrastructure error. Callable once per env; after Evaluate only Teardown is valid.
